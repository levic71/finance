...
?>
				<tr>
					...
					<td class="center aligned price_perf"><div>
						<button <?= $row['status'] == 1 ? 'data-tootik="'.$row['date_status'].'"' : "" ?> class="tiny ui <?= $row['objectif'] >= $cj ? "aaf-positive" : "orange" ?> button"><?= sprintf("%.2f", $row['objectif']).$curr ?></button>
						<label class="<?= $row['objectif'] >= $cj ? "aaf-positive" : "orange" ?>"><?= sprintf("%.2f%%", $perf) ?></label>
					</div></td>
					<td class="center aligned price_perf"><div>
