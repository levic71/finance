...
$entities_autorised = array("FOOTSPOTS" => 1);
...
function CreateChampionship($options) {
	...
	global $entities_autorised, $debug;

	$idc = 0;
	$log = "";
	$tmp = json_decode($options);
	...
	$desc    = isset($tmp->desc) ? trim($tmp->desc) : "";
    ...
    if (isset($tmp->teams) && is_array($tmp->teams))
    {
        ...
        {
			$insert = "INSERT INTO jb_equipes (id_champ, nom, photo, commentaire, external_id) VALUES (".$idc.", '".$team->name."', '".$team->photo."', '".$team->comment."', ".$team->id.");";
			if ($debug) $log .= $insert."\n";
			$res = mysql_query($insert);
...
function DeleteChampionship($options) {
	...
	if (isset($tmp->id))
		...
		return "-2:Id championship id mandatory";
	...
	if ($row = mysql_fetch_array($res))
	{
		if ($row['total'] == 0)
		{
			return $log."-10:Wrong championship parameters".mysql_real_escape_string(htmlentities("é'"));
		}
	}
