...
{
	function FXListPresentation($tab)
	{
...
{
	function FXListChampionnatsActifs($only_nul_points = false)
	{
		...
		foreach($most_active as $c)
		{
			...
			{
				if ($c['points'] == 0)
					$tab [] = array($c['dt_creation'], "<A CLASS=\"blue icon_".$c['type']."\" HREF=\"championnat_acces.php?ref_champ=".$c['id']."\">".$c['nom']."</A>", $c['email'], $c['points']." points", "<input type=radio name=\"c_".$c['id']."\" ".($c['actif'] == 1 ? "checked=\"checked\"" : "")." value=\"1\" /> Oui <input type=radio name=\"c_".$c['id']."\" ".($c['actif'] == 0 ? "checked=\"checked\"" : "")." value=\"0\" /> Non");
			}
			else
				$tab [] = array($c['dt_creation'], "<A CLASS=\"blue icon_".$c['type']."\" HREF=\"championnat_acces.php?ref_champ=".$c['id']."\">".$c['nom']."</A>", $c['email'], $c['points']." points", "<input type=radio name=\"c_".$c['id']."\" ".($c['actif'] == 1 ? "checked=\"checked\"" : "")." value=\"1\" /> Oui <input type=radio name=\"c_".$c['id']."\" ".($c['actif'] == 0 ? "checked=\"checked\"" : "")." value=\"0\" /> Non");
		}
        $fxbody = new FXBodyArray($tab, _FXLIST_FULL_);
        $this->FXList($fxbody);
        $this->FXSetColumnsName(array("Date", "Nom", "Email", "Points", "Actif"));
...
{
	function FXListForum($championnat, $admin = false, $dual = 0)
	{
		$filtre = "";
...
class FXListForumWhois extends FXList
{
	...
	function FXListForumWhois($championnat)
	{
...
{
	function FXListSaisons($championnat, $admin = false)
	{
		if ($admin)
		{
...
{
	function FXListPlayers($championnat, $type_championnat, $id_saison, $admin = false, $stat_access = true, $delta = "")
	{
		$filtre = "";
		...
		else
		{
			...
			$res = dbc::execSql($req);
			while ($equipes = mysql_fetch_array($res))
				if ($equipes['joueurs'] != "") $lst .= ($lst == "" ? "" : ",").$equipes['joueurs'];

			$filtre = ($lst != "" ? " AND id IN (".str_replace("|", ",", $lst).")" : "");
			*/
			$filtre = ($saison['joueurs'] != "" ? " AND id IN (".$saison['joueurs'].")" : "");
		}
...
{
	function FXListBestPlayers($best_teams, $option = "0")
	{
...
class FXListTeams extends FXList
{
	...
	function FXListTeams($championnat, $type, $id_saison, $admin = false)
	{
...
class FXListJournees extends FXList
{
	...
	function FXListJournees($championnat, $type_championnat, $admin = false)
	{
		$this->admin = $admin;
		if ($admin)
		{
...
class FXListStatsTeams extends FXList
{
	...
	function FXListStatsTeams($best_teams)
	{
		$t = array();
		reset($best_teams);
...
class FXListStatsAttDef extends FXList
{
	...
	function FXListStatsAttDef($best_att_def, $attaque = 0)
	{
		$t = array();
		reset($best_att_def);
...
class FXListStatsTeamsII extends FXList
{
	function FXDisplayColumnsName()
	{
		...
		if (sess_context::getGestionMatchsNul() == 0)         HTMLTable::printCell("Diff",      $this->c3_title, "", "center", _CELLBORDER_SE_);
        HTMLTable::printCell("Marqués",   $this->color_title_column, "", "center", _CELLBORDER_SE_);
        HTMLTable::printCell("Encaissés", $this->color_title_column, "", "center", _CELLBORDER_SE_);
        HTMLTable::printCell("Diff",      $this->color_title_column, "", "center", _CELLBORDER_SE_);
	}

	function FXListStatsTeamsII($best_teams, $xmloutput = false)
	{
		$t = array();
		reset($best_teams);
		foreach($best_teams as $team)
		...
		{
			$this->FXSetColumnsDisplayed(array("nom", "points", "matchs_joues", "matchs_gagnes", "matchs_nuls", "matchs_perdus", "sets_joues", "sets_gagnes", "sets_nuls", "sets_perdus", "buts_marques", "buts_encaisses", "diff"));
	        $this->FXSetColumnsColor(array("", $this->c1_column, $this->c2_column, $this->c2_column, $this->c2_column, $this->c2_column, $this->c3_column, $this->c3_column, $this->c3_column, $this->c3_column, ""));
		}
		else
		{
			$this->FXSetColumnsDisplayed(array("nom", "points", "matchs_joues", "matchs_gagnes", "matchs_perdus", "sets_joues", "sets_gagnes", "sets_perdus", "sets_diff", "buts_marques", "buts_encaisses", "diff"));
	        $this->FXSetColumnsColor(array("", $this->c1_column, $this->c2_column, $this->c2_column, $this->c2_column, $this->c3_column, $this->c3_column, $this->c3_column, $this->c3_column, ""));
		}
        $this->FXSetColumnsWidth(array("40%", "", ""));
	}
...
{
	function FXListBestTeams($best_teams, $delta = "")
	{
...
{
	function FXListMostOnGround($most_matchs, $delta = "")
	{
...
{
	function FXListFannys($championnat, $requete, $delta = "")
	{
...
{
	function FXListFannysJoueur($championnat, $id_joueur = "", $delta = "")
	{
		if ($id_joueur == "")
...
{
	function FXListFannysEquipe($championnat, $id_equipe = "", $delta = "")
	{
		if ($id_equipe == "")
        	$requete = "SELECT m.fanny, m.nbset, j.date date, e1.nom nom1, m.resultat resultat, e2.nom nom2 FROM jb_matchs m, jb_equipes e1, jb_equipes e2, jb_journees j WHERE m.fanny=1 AND e1.id=m.id_equipe1 AND e2.id=m.id_equipe2 AND m.id_journee=j.id AND m.id_champ=".$championnat." ORDER BY date DESC";
		else
...
{
	function FXListMatchs($championnat, $id_journee = "", $admin = false, $filtre = "")
	{
		if ($admin)
		{
...
class FXListMatchsPoules extends FXListMatchsGen
{
	function FXListMatchsPoules($championnat, $id_journee = "", $admin = false, $filtre = "")
	{
		...
		{
	        $requete = "SELECT m.penaltys, m.prolongation, m.id match_id, j.date mdate, m.match_joue, m.id_equipe1, m.id_equipe2, m.niveau niveau, j.date date, e1.nom nom1, m.resultat resultat, e2.nom nom2, m.fanny, m.nbset FROM jb_matchs m, jb_equipes e1, jb_equipes e2, jb_journees j WHERE e1.id=m.id_equipe1 AND e2.id=m.id_equipe2 AND m.id_journee=j.id AND m.id_journee=".$id_journee." AND m.id_champ=".$championnat." ".$filtre." ORDER BY niveau DESC";
	        $this->fxbody = new FXBodySQL($requete, _FXLIST_FULL_);
			$this->preformat($championnat, $id_journee);
			$this->formatForDisplay();
	        $this->FXList($this->fxbody);
...
{
	function FXListMatchsPlayOff($championnat, $id_journee = "", $phase_finale = 16, $admin = false, $type_matchs)
	{
		if ($admin)
		{
	        $requete = "SELECT m.penaltys, m.prolongation, m.match_joue, m.niveau niveau, m.fanny, m.nbset, CONCAT('<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0><TR><TD><A HREF=# onClick=\"javascript:modifier_match(\'+WHERE+id%3D',m.id,'\', \'\', \'', m.niveau,'\');\"><IMG SRC=../images/small_edit2.gif BORDER=0 /></A></TD><TD><A HREF=# onClick=\"javascript:supprimer_match(\'+WHERE+id%3D',m.id,'\');\"><IMG SRC=../images/small_poubelle.gif BORDER=0 /></A></TD></TABLE>') action, j.date date, e1.nom nom1, m.resultat resultat, e2.nom nom2 FROM jb_matchs m, jb_equipes e1, jb_equipes e2, jb_journees j WHERE e1.id=m.id_equipe1 AND e2.id=m.id_equipe2 AND m.id_journee=j.id AND m.id_journee=".$id_journee." AND m.id_champ=".$championnat." AND niveau LIKE '".$type_matchs."|%' ORDER BY niveau DESC";
			$this->fxbody = new FXBodySQL($requete, _FXLIST_FULL_);
			$this->completeList($admin, $phase_finale);
			$this->formatForDisplay();
	        $this->FXList($this->fxbody);
	        $this->FXSetTitle("Matchs");
		...
		{
	        $requete = "SELECT m.penaltys, m.prolongation, m.match_joue, m.niveau niveau, j.date date, e1.nom nom1, m.resultat resultat, e2.nom nom2, m.fanny, m.nbset FROM jb_matchs m, jb_equipes e1, jb_equipes e2, jb_journees j WHERE e1.id=m.id_equipe1 AND e2.id=m.id_equipe2 AND m.id_journee=j.id AND m.id_journee=".$id_journee." AND m.id_champ=".$championnat." AND niveau LIKE '".$type_matchs."|%' ORDER BY niveau DESC";
	        $this->fxbody = new FXBodySQL($requete, _FXLIST_FULL_);
			$this->completeList($admin, $phase_finale);
			$this->formatForDisplay();
	        $this->FXList($this->fxbody);
	...
	function completeList($admin, $phase_finale)
	{
		...
		while($i >= 1)
		{
			for($j=1; $j <= $i; $j++)
			{
				$action = $admin ? "<A HREF=\"matchs_ajouter.php?options_type_matchs=F|".$i."|".$j."\"><IMG SRC=../images/small_edit2.gif BORDER=0 /></A>" : "";
				$res["F|".$i."|".$j] = array("niveau" => $libelle_phase_finale[$i], "nom1" => "-", "resultat" => "0/0", "nom2" => "-", "action" => $action, "nbset" => 1, "fanny" => 0);
			}
		...
		foreach($this->fxbody->tab as $elt)
		{
			...
			$elt['niveau'] = $libelle_phase_finale[$items[1]];
			$res[$tmp] = $elt;
		}
...
class FXListMatchsPlayOffII extends FXListPresentation
{
	...
	function FXListMatchsPlayOffII($championnat, $id_journee = "", $phase_finale = 16, $admin = false, $type_matchs)
	{
		$this->championnat  = $championnat;
	...
	function initData()
	{
		...
		$res = dbc::execSQL($select);
		while($row_matchs = mysql_fetch_array($res))
...
{
	function FXListMatchsClassementTournoi($championnat, $id_journee = "", $nb_equipes = 0, $admin = false, $filtre = "")
	{
		if ($admin)
		{
	...
	function completeList($admin, $nb_equipes)
	{
		...
		for($i=3; $i <= $nb_equipes; $i++)
		{
			if (($i % 2) != 0)
			{
				...
				$res["C|".$i] = array("prolongation" => "0", "penaltys" => "", "match_joue" => "0", "niveau" => $i." ième place", "nom1" => "-", "resultat" => "0/0", "nom2" => "-", "action" => $action, "nbset" => 1, "fanny" => 0);
			}
		}
...
class FXListJourneeVirtuelle extends FXListPresentation
{
	...
	function FXListJourneeVirtuelle($championnat, $saison, $datas = "", $admin = false)
	{
		$this->championnat = $championnat;
		$this->saison      = $saison;
		$this->datas       = array();
...
class FXListClassementJourneeTournoi extends FXListPresentation
{
	...
	function FXListClassementJourneeTournoi($championnat, $id_saison, $journee)
	{
		$this->championnat = $championnat;
		$this->id_saison   = $id_saison;
...
class FXListClassementGeneralTournoi extends FXListPresentation
{
	...
	function FXListClassementGeneralTournoi($championnat, $id_saison, $classement)
	{
		$this->championnat = $championnat;
		$this->id_saison   = $id_saison;
...
class FXListMatchsStatsJoueurs extends FXList
{
	...
	function FXListMatchsStatsJoueurs($classement, $joueurs)
	{
		$t = array();
...
class FXListMatchsStatsEquipes extends FXList
{
	...
	function FXListMatchsStatsEquipes($classement, $equipes)
	{
		$t = array();
...
class FXListMatchsStatsEquipesLight extends FXList
{
	...
	function FXListMatchsStatsEquipesLight($classement, $equipes)
	{
		$t = array();
...
class FXListStatsJoueurs extends FXList
{
	...
	function FXListStatsJoueurs($sgb, $id_joueur = "-1")
	{
		$this->sgb = $sgb;
	...
	function getStatsAllJoueurs()
	{
		...
		if (count($regulier) == 0)
			...
			return array_merge($regulier, _FXSEPARATORWITHINIT_, $okaz);
	}
	function getXmlClassement()
	{
        // Synthèse des stats avec tri sur tournoi_points
...
class FXListStatsTournoiJoueurs extends FXList
{
	...
	function FXListStatsTournoiJoueurs($sgb, $id_joueur = "-1")
	{
		$this->sgb = $sgb;
		$this->nom_joueurs   = $sgb->getPlayersName();
		$this->stats_joueurs = $sgb->getStatsPlayers();
...
class FXListStatsConfrontations extends FXList
{
	...
	function FXListStatsConfrontations($championnat, $id_equipe, $delta = "")
	{
		$this->championnat = $championnat;
...
class FXListAlbumsThemes extends FXList
{
	...
	function FXListAlbumsThemes($championnat, $admin = false)
	{
		$this->championnat = $championnat;
		$this->admin       = $admin;
...
class FXListAlbums extends FXList
{
	...
	function FXListAlbums($championnat, $theme, $admin = false)
	{
		$this->championnat = $championnat;
		$this->admin       = $admin;
		$this->theme       = $theme;
