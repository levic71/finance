...
class SQLServices
{
	...
	function SQLServices($championnat)
	{
...
class SQLChampionnatsServices extends SQLServices
{
	...
	function getNbJourneesGen($where = "")
	{
		...
		if ($all_saisons != "")
		{
			$req = "SELECT COUNT(*) total FROM jb_journees ".($this->championnat != -1 ? "WHERE id_champ IN (".$this->getAllSaisons($this->championnat).") ".$where : "");
			$res = dbc::execSQL($req);
			if ($row = mysqli_fetch_array($res)) $nb = $row['total'];
		}
		else
	...
	function getNbMatchsGen($where = "")
	{
		...
		if ($all_saisons != "")
		{
			$req = "SELECT COUNT(*) total FROM jb_matchs ".($this->championnat != -1 ? "WHERE id_champ IN (".$this->getAllSaisons($this->championnat).") ".$where : "");
			$res = dbc::execSQL($req);
			if ($row = mysqli_fetch_array($res)) $nb = $row['total'];
		}
		else
	...
	{
		return $this->getNbMatchsGen("");
	}

	function getNbMessagesGen($where = "")
	{
		$nb = 0;

		$req = "SELECT COUNT(*) total FROM jb_forum WHERE del=0 ".($this->championnat != -1 ? "AND id_champ=".$this->championnat." ".$where : "");
		$res = dbc::execSQL($req);
		if ($row = mysqli_fetch_array($res)) $nb = $row['total'];
	...
	{
		return $this->getNbMessagesGen("");
	}

	function getNbJourneesTranche1()
	{
		return $this->getNbJourneesGen(" AND TO_DAYS(now())-TO_DAYS(date) < 30");
	}

	function getNbJourneesTranche2()
	{
		return $this->getNbJourneesGen(" AND TO_DAYS(now())-TO_DAYS(date) < 60");
	}

	function getNbJourneesTranche3()
	{
		return $this->getNbJourneesGen(" AND TO_DAYS(now())-TO_DAYS(date) < 120");
	}

	function getNbJourneesTranche4()
	{
		return $this->getNbJourneesGen(" AND TO_DAYS(now())-TO_DAYS(date) < 240");
	}
	...
	function getMostActiveChampionnats()
	{
		...
		while($row = mysqli_fetch_array($res))
		{
			...
			$nb_journees_T4 = $this->getNbJourneesTranche4() - $nb_journees_T3 - $nb_journees_T2 - $nb_journees_T1;
			$nb_journees_T5 = $nb_journees_total - $nb_journees_T4 - $nb_journees_T3 - $nb_journees_T2 - $nb_journees_T1;
			$pointsmax = $nb_journees_total * 50 + $this->getNbMatchs() * 3 + $this->getNbMessages();
			$points = (
					$nb_journees_T1 * 25 * 1) +
					floor($nb_journees_T2 * 25 * 0.75) + 
					floor($nb_journees_T3 * 25 * 0.50) + 
					floor($nb_journees_T4 * 25 * 0.25)+ 
					floor($nb_journees_T5 * 25 * 0.125) + 
//					$this->getNbMatchs() * 3 + 
					floor($this->getNbMessages() * 0.5
					);
			
			if ($nb_journees_T1 == 0 && $nb_journees_T2 == 0 && $nb_journees_T3 == 0 && $nb_journees_T4 == 0)
				$points = floor($points / 2);

			$lst[$row['id']] = array('id' => $row['id'], 'nom' => $row['nom'], 'points' => $points, 'pointsmax' => $pointsmax, 'type' => $row['type'], 'dt_creation' => $row['dt_creation'], 'actif' => $row['actif'], 'email' => $row['email']);
	...
	function getLastChampionnatsCrees()
	{
		...
		while($row = mysqli_fetch_array($res))
		{
			...
			$points = ($nb_journees_T1 * 50 * 1) + floor($nb_journees_T2 * 50 * 0.50) + floor($nb_journees_T3 * 50 * 0.25) + floor($nb_journees_T4 * 50 * 0.125) + $this->getNbMatchs() * 3 + floor($this->getNbMessages() * 0.5);
			$row['points'] = $points;
			$lst[$row['id']] = $row;
		}
...
class SQLSaisonsServices extends SQLServices
{
	...
	function SQLSaisonsServices($championnat, $saison = -1)
	{
		$this->SQLServices($championnat);
		$this->saison = $saison;
	}
...
{
	function SQLJoueursServices($championnat)
	{
		$this->SQLServices($championnat);
	}
	...
	function getListeJoueursFromIds($ids = "")
	{
		...
		$req = "SELECT * FROM jb_joueurs WHERE id_champ=".$this->championnat." AND id IN (".$ids.")";
		$res = dbc::execSql($req);
	...
	function getListeJoueursByEquipes($liste_equipes)
	{
		...
		{
			$j_selected = array();
			$req = "SELECT * FROM jb_equipes WHERE id IN (".$liste_equipes.")";
			$res = dbc::execSQL($req);
			while($row = mysqli_fetch_array($res))
			{
				if ($row['nb_joueurs'] >= 2)
...
{
	function SQLEquipesServices($championnat)
	{
		$this->SQLServices($championnat);
	}
	...
	function getListeEquipes($ids = "")
	{
		...
		$req = "SELECT * FROM jb_equipes WHERE id_champ=".$this->championnat." ".($all_equipes == "" ? "" : "AND id IN (".$all_equipes.")")." ORDER BY nom ASC";
		$res = dbc::execSql($req);
...
class SQLJourneesServices extends SQLServices
{
	...
	function SQLJourneesServices($championnat, $journee)
	{
		$this->SQLServices($championnat);
	...
	function getListeJourneesGen($date_debut, $date_fin, $option = 0)
	{
		...
		$req = "SELECT * from jb_journees WHERE id_champ IN (".$this->championnat.") AND date between '".$date_debut."' AND  '".$date_fin."';";
		$res = dbc::execSQL($req);
		while($row = mysqli_fetch_array($res))
		{
			if ($option == 0)
...
class SQLMatchsServices extends SQLServices
{
	...
	function SQLMatchsServices($championnat, $journee, $match)
	{
		$this->SQLServices($championnat);
		$this->journee = $journee;
		$this->match   = $match;
...
{
	function SQLForumServices($championnat)
	{
		$this->SQLServices($championnat);
	}
...
{
	function SQLAlbumsServices($championnat = -1)
	{
		$this->SQLServices($championnat);
	}
...
{
	function SQLActualitesServices($championnat = -1)
	{
		$this->SQLServices($championnat);
	}
...
{
	function SQLVideossServices($championnat = -1)
	{
		$this->SQLServices($championnat);
	}
...
class SQLAlbumsThemesServices extends SQLServices
{
	...
	function SQLAlbumsThemesServices($championnat = -1, $id_theme = -1)
	{
		$this->SQLServices($championnat);
