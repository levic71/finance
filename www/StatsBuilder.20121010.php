...
class StatMatch
{
	...
	function StatMatch($score, $nbSet)
	{
...
class StatsJourneeBuilder
{
	...
	function StatsJourneeBuilder($championnat, $journee, $type_championnat = _TYPE_LIBRE_, $filtre_matchs = "")
	{
	...
	function ComputeStats()
	{
		...
		while($row = mysqli_fetch_array($res, MYSQLI_ASSOC))
		{
			...
			$j1 = $this->statsTeams[$e1]->defenseur; // Défenseur equipe 1
			$j2 = $this->statsTeams[$e1]->attaquant; // Attaquant equipe 1
			$j3 = $this->statsTeams[$e2]->defenseur; // Défenseur equipe 2
			$j4 = $this->statsTeams[$e2]->attaquant; // Attaquant equipe 2
			...
			if ($score != -1 && $score != -2)
			{
				...
				if ($row['nbset'] >= 2)
					$this->cumulScores($j1, $j2, $j3, $j4, $e1, $e2, $score[1][0], $score[1][1]);
				if ($row['nbset'] >= 3)
					$this->cumulScores($j1, $j2, $j3, $j4, $e1, $e2, $score[2][0], $score[2][1]);
				if ($row['nbset'] >= 4)
					$this->cumulScores($j1, $j2, $j3, $j4, $e1, $e2, $score[3][0], $score[3][1]);
				if ($row['nbset'] == 5)
					$this->cumulScores($j1, $j2, $j3, $j4, $e1, $e2, $score[4][0], $score[4][1]);
			}
			...
			$this->statsTeams[$e1]->points = ($this->statsTeams[$e1]->matchs_gagnes * $this->val_victoire) + ($this->statsTeams[$e1]->matchs_perdus * $this->val_defaite) + $this->gestion_nul * ($this->statsTeams[$e1]->matchs_nuls * $this->val_nul);
			$this->statsTeams[$e2]->points = ($this->statsTeams[$e2]->matchs_gagnes * $this->val_victoire) + ($this->statsTeams[$e2]->matchs_perdus * $this->val_defaite) + $this->gestion_nul * ($this->statsTeams[$e2]->matchs_nuls * $this->val_nul);
	...
	function getClassementPlayers()
	{
		...
		while(list($cle, $val) = each($this->statsJoueurs))
			$classement[$cle] = sprintf("%03s", $this->statsJoueurs[$cle]->matchs_gagnes).sprintf("%03s", 200+$this->statsJoueurs[$cle]->diff).sprintf("%03s", 200+$this->statsJoueurs[$cle]->buts_marques);
...
class StatsGlobalBuilder
{
	...
	function StatsGlobalBuilder($championnat, $type_championnat = _TYPE_LIBRE_, $type_sport = _TS_JORKYBALL_)
	{
		$d1 = date("U");
		$this->championnat      = $championnat;
		$this->type_championnat = $type_championnat;
...
//	    while(list($id, $val) = each($this->stats_equipes)) $this->stats_equipes[$val]->evol_classement[$date_journee] = "";
	...
	function calculeIndiceForme($id)
	{
		...
		$alt = "Etat de forme sur les 4 dernières journées";
       	if ($indice >= -5 && $indice <= 5)
       		$this->stats_joueurs[$id]->forme_indice = "<IMG SRC=../images/fleches/fleche3.gif ALT=\"".$alt."\" BORDER=0 />";
       	else if ($indice < -20)
       		$this->stats_joueurs[$id]->forme_indice = "<IMG SRC=../images/fleches/fleche1.gif ALT=\"".$alt."\" BORDER=0 />";
		else if ($indice < -5)
			$this->stats_joueurs[$id]->forme_indice = "<IMG SRC=../images/fleches/fleche2.gif ALT=\"".$alt."\" BORDER=0 />";
		else if ($indice > 20)
	    	$this->stats_joueurs[$id]->forme_indice = "<IMG SRC=../images/fleches/fleche5.gif ALT=\"".$alt."\" BORDER=0 />";
		else if ($indice > 5)
	    	$this->stats_joueurs[$id]->forme_indice = "<IMG SRC=../images/fleches/fleche4.gif ALT=\"".$alt."\" BORDER=0 />";
	}
	...
	function calculeFormeLastDay($id)
	{
        ...
        if ($indice >= -5 && $indice <= 5)
         	$this->stats_joueurs[$id]->forme_last_indice = "<IMG SRC=../images/fleches/fleche3.gif ALT=\"".$alt."\" BORDER=0 />";
        else if ($indice < -20)
        	$this->stats_joueurs[$id]->forme_last_indice = "<IMG SRC=../images/fleches/fleche1.gif ALT=\"".$alt."\" BORDER=0 />";
       	else if ($indice < -5)
       		$this->stats_joueurs[$id]->forme_last_indice = "<IMG SRC=../images/fleches/fleche2.gif ALT=\"".$alt."\" BORDER=0 />";
       	else if ($indice > 20)
       		$this->stats_joueurs[$id]->forme_last_indice = "<IMG SRC=../images/fleches/fleche5.gif ALT=\"".$alt."\" BORDER=0 />";
       	else if ($indice > 5)
      		$this->stats_joueurs[$id]->forme_last_indice = "<IMG SRC=../images/fleches/fleche4.gif ALT=\"".$alt."\" BORDER=0 />";
	}
