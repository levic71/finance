...
<form action="gestion_chroniques.php" method="post" enctype="multipart/form-data">
...
echo "<tr><td>";
$fxlist = new FXListChroniques();
$fxlist->FXSetPagination("gestion_chroniques.php");
$fxlist->FXDisplay();
echo "</td>";
...
if (!(isset($FXOption) && $FXOption == _FXLIST_EXPORT_))
{
	...
	echo "	<td align=\"right\"><input type=\"submit\" name=\"bouton\" value=\"Ajouter une chronique\" onclick=\"javascript:ajouter_item();\" /></td>";
	echo "</table></td>";
}
...
{
    document.forms[0].action = 'gestion_chroniques_ajouter.php';
}
...
{
	if (!confirm('Etes-vous de vouloir supprimer cette chronique ?'))
		return false;

	document.forms[0].type_action.value = action;
	document.forms[0].pkeys_where.value = pkeys;
    document.forms[0].action = 'gestion_chroniques_supprimer_do.php';
