...
echo "<tr><td>";
$fxlist = new FXListVideos();
$fxlist->FXSetPagination("gestion_videos.php");
$fxlist->FXDisplay();
echo "</td>";
...
if (!(isset($FXOption) && $FXOption == _FXLIST_EXPORT_))
{
	...
	echo "	<td align=\"right\"><input type=\"submit\" name=\"bouton\" value=\"Ajouter une vidéo\" onclick=\"javascript:ajouter_item();\" /></td>";
	echo "</table></td>";
}
...
{
    document.forms[0].action = 'gestion_videos_ajouter.php';
}
...
{
	if (!confirm('Etes-vous de vouloir supprimer cette video ?'))
		return false;

	document.forms[0].type_action.value = action;
	document.forms[0].pkeys_where.value = pkeys;
    document.forms[0].action = 'gestion_videos_supprimer_do.php';
