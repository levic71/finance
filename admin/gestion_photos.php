...
<form action="gestion_photos.php" method="post" enctype="multipart/form-data">
...
echo "<tr><td>";
$fxlist = new FXListPhotos();
$fxlist->FXSetPagination("gestion_photos.php");
$fxlist->FXDisplay();
echo "</td>";
...
if (!(isset($FXOption) && $FXOption == _FXLIST_EXPORT_))
{
	...
	echo "	<td align=\"right\"><input type=\"submit\" name=\"bouton\" value=\"Ajouter une photo\" onclick=\"javascript:ajouter_item();\" /></td>";
	echo "</table></td>";
}
...
{
    document.forms[0].action = 'gestion_photos_ajouter.php';
}
...
function supprimer_item(pkeys, action)
{
	...
	document.forms[0].type_action.value = action;
	document.forms[0].pkeys_where.value = pkeys;
    document.forms[0].action = 'gestion_photos_supprimer_do.php';
