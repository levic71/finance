...
<form action="gestion_astuces.php" method="post" enctype="multipart/form-data">
...
echo "<tr><td>";
$fxlist = new FXListAstuces();
$fxlist->FXSetPagination("gestion_astuces.php");
$fxlist->FXDisplay();
echo "</td>";
...
if (!(isset($FXOption) && $FXOption == _FXLIST_EXPORT_))
{
	...
	echo "	<td align=\"right\"><input type=\"submit\" name=\"bouton\" value=\"Ajouter une astuce\" onclick=\"javascript:ajouter_item();\" /></td>";
	echo "</table></td>";
}
...
{
    document.forms[0].action = 'gestion_astuces_ajouter.php';
}
...
{
	if (!confirm('Etes-vous de vouloir supprimer cette astuce ?'))
		return false;

	document.forms[0].type_action.value = action;
	document.forms[0].pkeys_where.value = pkeys;
    document.forms[0].action = 'gestion_astuces_supprimer_do.php';
