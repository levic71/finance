...
var calendar = function() {
	...
	gendays:function(d, m, y, dn) {
		...
		for(i=(1+blank); i <= (dim[m-1]+blank); i++) v[i-1] = { v: ((i-blank) < 10 ? '0' : '')+(i-blank)+'/'+((m < 10 ? '0' : '')+m)+'/'+y, l: (i-blank)+'', s: (parseInt(refd) == (i-blank) && parseInt(refm) == parseInt(m) && parseInt(refy) == parseInt(y) ? true : false) };
...
var counter = function() {
	...
	showSet:function(id) {
		...
		el(name+'_labelset').innerHTML = id == 0 ? "1er set" : (id+1)+"ième set";
	},
