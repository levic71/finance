...
<? if ($sess_context->isTournoiXDisplay()) { ?>
	<img id="more21" class="bt" onclick="toggle_all2(999);" src="img/icons/dark/appbar.add.png" />
	<img id="less21" class="bt" onclick="toggle_all2(<?= sess_context::getHomeListHeadcount() ?>);" src="img/icons/dark/appbar.minus.png" />
	<img id="more22" class="bt" onclick="toggle_all2(999);" src="img/icons/dark/appbar.add.png" />
	<img id="less22" class="bt" onclick="toggle_all2(<?= sess_context::getHomeListHeadcount() ?>);" src="img/icons/dark/appbar.minus.png" />
<? } else if ($sess_context->isFreeXDisplay()) { ?>
	<img id="more31" class="bt" onclick="toggle_all(999);" src="img/icons/dark/appbar.add.png" />
	<img id="less31" class="bt" onclick="toggle_all(<?= sess_context::getHomeListHeadcount() ?>);" src="img/icons/dark/appbar.minus.png" />
	<img id="more32" class="bt" onclick="toggle_all(999);" src="img/icons/dark/appbar.add.png" />
	<img id="less32" class="bt" onclick="toggle_all(<?= sess_context::getHomeListHeadcount() ?>);" src="img/icons/dark/appbar.minus.png" />
<? } else { ?>
	<img id="more4" class="bt" onclick="show_elts('matches', 999);" src="img/icons/dark/appbar.add.png" />
...
<?= Wrapper::displayMatchesStats($journee['id'], true); ?>
<div id="classement" class="underline" style="margin: 5px 10px;"></div>
<script>
...
swap_box = function(name) {
	hide(choices.getSelection(name) == 1 ? 'box3' : 'matches'); hide(choices.getSelection(name) == 1 ? 'more31' : 'more32'); hide(choices.getSelection(name) == 1 ? 'less31' : 'less32');
	show(choices.getSelection(name) == 1 ? 'matches' : 'box3'); show(choices.getSelection(name) == 1 ? 'more32' : 'more31'); show(choices.getSelection(name) == 1 ? 'less32' : 'less31');
	show_elts(choices.getSelection(name) == 1 ? 'matches' : 'box3', compact3, <?= sess_context::getHomeListHeadcount() ?>, choices.getSelection(name) == 1 ? 'more32' : 'more31', choices.getSelection(name) == 1 ? 'less32' : 'less31');
}

toggle_all = function(nb) {
	compact3 = nb;
	show_elts('box3', compact3, <?= sess_context::getHomeListHeadcount() ?>, 'more31', 'less31');
...
<table id="classementjournee" cellspacing="0" cellpadding="0" class="jkgrid matchespoules" style="width: 100%;">
<? journee_tournoi_classement($journee, $saison_id, $real_id, $sess_context->getGestionMatchsNul()); ?>
</table>
...
<thead>
<tr>
	<th class="c1"><div>&nbsp;</div></th>
	<th class="c2"><div>Equipe</div></th>
	<th class="c3"><div>Pts</div></th>
	<th class="c4"><div>Diff</div></th>
</tr>
</thead>
...
//			if ($i > 10) break;
			...
			if ($idt != '' && $idt != 0) {
?><tr id="tr_<?= $i ?>" class="clickonit" onclick="mm({action:'stats', idt:'<?= $idt ?>'});">
		<td class="c1"><div><?= $i++ ?></div></td>
		<td class="c2"><div><?= $eq ?></div></td>
		<td class="c3"><div><button class="button bigrounded gray"><?= $item['2'] ?></button></div></td>
		<td class="c4"><div><button class="button bigrounded <?= ($matchs_nul ? $item['9'] : $item['12']) >= 0 ? "green" : "red" ?>"><?= $matchs_nul ? $item['9'] : $item['12'] ?></button></div></td>
...
<?
	...
	for($x = $i; $x <= sess_context::getHomeListHeadcount(); $x++)
		echo "<tr id=\"tr_".$x."\"><td class=\"c1\"><div>".$x."</div></td><td class=\"c2\"><div>-</div></td><td class=\"c3\"><div><button class=\"button bigrounded disable\">0</button></div></td><td class=\"c4\"><div><button class=\"button bigrounded disable\">0</button></div></td></tr>";
