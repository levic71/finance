...
{
	if (str.length == 0) { alert('Le champ <'+label+'> ne doit pas �tre vide'); return false; }
	if (!(str.length == 10)) { alert('Le champ <'+label+'> doit �tre de la forme JJ/MM/AAAA'); return false; }
	var jour=str.substring(0, 2); var mois=str.substring(3, 5); var year=str.substring(6, 10);
	if (jour > 31 || jour < 1 || mois < 1 || mois > 12) { alert('Le champ <'+label+'> doit �tre de la forme JJ/MM/AAAA'); return false; }
	return true;
}

check_num = function(num, label, min, max)
{
	if (num.length == 0) { alert('Le champ <'+label+'> ne doit pas �tre vide'); return false; }
	for(var i=0; i < num.length; i++)
	{
		var car=num.substring(i, i+1);
		if (!(car >= "0" && car <= "9")) { alert('Le champ <'+label+'> doit �tre num�rique'); return false; }
	}
	if (num > max || num < min) { alert('Le champ <'+label+'> doit �tre compris entre '+min+' et '+max); return false; }
	return true;
}
...
{
	if (str.length == 0) { alert('Le champ <'+label+'> ne doit pas �tre vide'); return false; }
	if (size != -1 && str.length < size) { alert('Le champ <'+label+'> doit �tre compos� d\'au moins '+size+' caract�res alphanum�riques'); return false; }
	for(var i=0; i < str.length; i++)
	{
		var car=str.substring(i, i+1);
		if (type == 0)
			if (!isacar(car)) { alert('Le champ <'+label+'> doit �tre alphanum�rique'); return false; }
		else
			if (!isacarext(car)) { alert('Le champ <'+label+'> doit �tre alphanum�rique'); return false; }
	}
	return true;
}
