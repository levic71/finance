...
var numbers = function() {
	...
	picker:function(args) {
		...
		numbers.setEnv(args);
		if (name == '') return;
		if (name.indexOf('_') != -1) { alert("'_' non tol�r� !"); return; }
...
var calendar = function() {
	...
	setEnv:function(args) {
		...
		c1    = args.c1||'orange';
		c2    = args.c2||'blue';
		mn    = ['Janvier','F�vrier','Mars','Avril','Mai','Juin','Juillet','Ao�t','Septembre','Octobre','Novembre','D�cembre'];
	...
	click:function(id) {
		...
		else
		{
			...
			else if (multiple)
			{
				...
				if (callback != '') {	// A faire dans les autres cas en fonction comportement souhait� (peut etre ... pas sur !)
...
var counter = function() {
	...
	insertCommentElt:function(time, type, comment) {
		...
		var udt = '<td class="edit"><a onclick="alert(\'Pas encore impl�ment�\');"><img src="img/pencil_16.png" /></a></td><td class="del"><a onclick="counter.delComment(event);"><img src="img/delete_16.png" /></a></td>';
	...
	showSet:function(id) {
		...
		el(name+'_labelset').innerHTML = id == 0 ? "1er set" : (id+1)+"i�me set";
	},
...
drawAnalytics = function (args) {
    ...
    var labels = args.labels||new Array(),
        ...
        lblext = args.lblext||"% matchs gagn�s",
